...
<html lang="en">
<head>
<title> Allotment notification</title>
...
<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#tog" aria-controls="tog" aria-expanded="false" aria-label="Toggle navigation">
				...
				<ul class="navbar-nav " style="font-size: 18px; margin-right:-8%; ">
					...
					<li class="nav-item">
						<a class="nav-link " href="message_user1.php"><i class="fas fa-envelope"></i>.Feedback&nbsp&nbsp<span class="sr-only">(current)</span></a>
                    </li>
					<li class="nav-item active">
						<a class="nav-link" href="message_user1.php"><i class="fas fa-bell"></i>.notice</a>
                    </li>
        ...
        <ul class="navbar-nav">
					...
					<a class="dropdown-item" href="#" style="font-size: 18px;font-weight:bold;" data-toggle="modal" data-target="#logout" > signout <i class="fas fa-sign-out-alt"></i></a>
                        ...
                        <div id="container" class="row justify-content-md-center" style="margin-top:20px">
...
<div class="row" style="width:100%;"> 
   ...
   <table class="table table-hover table-inverse  table-bordered table-center table-striped mt-2 border-top" style="font-size:20px;" id='feedback'>
   <thead class="thread-inverse">
		   ...
		   <tbody>
			  <?php
			  $count = 1;
				  $view = "SELECT * FROM notice WHERE Student_id ='{$_SESSION['roll']}';";
				  $vdb = mysqli_query($conn,$view);
                   while($row = mysqli_fetch_array($vdb)){
					   echo'<tr>
					   <td>'.$count.'</td>
							<td>'.$row['N_subject'].'</td>
							<td>'.$row['N_description'].'</td>
							<td>'.$row['Date_sent'].'</td>
