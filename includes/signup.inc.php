  ...
  if(!preg_match("/^[a-zA-Z\/0-9]*$/",$roll)){
    ...
    if(!mysqli_stmt_prepare($stmt, $sql)){
      ...
      if ($resultCheck > 0) {
        ...
        $sql = "INSERT INTO Student (Student_id, Fname, Lname,Gender, Mob_no,Email, Dept, Year_of_study, Pwd,Date_registered) VALUES (?,?,?, ?, ?, ?, ?, ?, ?,?)";
        $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt, $sql)){
          header("Location: ../signup.php?error=sqlerror");
          exit();
          ...
          mysqli_stmt_bind_param($stmt, "ssssssssss",$roll, $fname, $lname,$gen, $mobile,$mail, $dept, $year, $hashedPwd,$date_time);
          mysqli_stmt_execute($stmt);
