...
<button class="btn btn-link btn-lg order-1 order-lg-1 "  style="font-weight:20px" id="sidebarToggle" href="#"><i class="fas fa-bars" style="color: white; font-size:large; cursor:pointer"></i></button>
  ...
  <div class="collapse navbar-collapse justify-content-md-center"  style="margin-left:42%;" id="navbarsExample08">
        ...
        <ol class="breadcrumb bg-info mb-1 ">
                           ...
                           <li class="breadcrumb-item active">Complaint_report</li>
                       </ol>
                ...
                <div class="table-responsive">
						...
						<table id="example" class="table table-hover table-inverse  table-bordered table-center table-striped mt-2 border-top" style="font-size:24px; align-content:center;width:100%;">  
							...
							<tr class="text-secondary">
													...
													<th>Date</th>
													<th>Complaints recieved</th>
													<th>Electrical Faults</th>
													<th>Water/Toilet Faults</th>
													<th>Furniture faults</th>
													<th>Replied complaints</th>
													<th>Unreplied Complaints</th>
													<th>mailed faults</th>
													<th>unmailed faults </th>
													
												</tr>
							</thead>
							<tbody>
								<?php
									$select = "SELECT * FROM complaint_report";
									$sdb = mysqli_query($conn,$select);
								
										while($row = mysqli_fetch_array($sdb)){
											echo'	
												<tr>
												<td>'.$row['Creport_date'].'</td>
												<td>'.$row['complaintsFiled'].'</td>
												<td>'.$row['electricalFaults'].'</td>
												<td>'.$row['furnitureFaults'].'</td>
												<td>'.$row['plumberFaults'].'</td>
												<td>'.$row['repliedmsg'].'</td>
												<td>'.$row['unrepliedmgs'].'</td>
												<td>'.$row['mailed'].'</td>
												<td>'.$row['unmailed'].'</td>
												</tr>
												';
